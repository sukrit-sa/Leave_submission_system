...
<body>
    ...
    $sql_leaves = "SELECT leaves.leavesid, leaves.leavestatus, leaves.leavetype, leaves.leavestart, leaves.leaveend, 
                   leavetype.leavetypename, leaves.note, leaves.day
                   FROM leaves 
                   JOIN leavetype ON leaves.leavetype = leavetype.leavetypeid 
                   WHERE leaves.employeesid = '$userID'
                   AND leaves.leavestatus IN ('รอหัวหน้าอนุมัติ', 'รอรองผอ.อนุมัติ', 'รอผอ.อนุมัติ', 'อนุมัติ', 'รออนุมัติ') AND leaves.note = 'ลา'";
                        ...
                        <table id="leaveTable" class="table table-striped table-bordered dt-responsive nowrap" style="width:100%">
                            ...
                            <tbody>
                                <?php foreach ($leaves as $leave): ?>
                                    <?php
                                    // แปลงวันที่เริ่มลาเป็นรูปแบบ d/m/Y (ค.ศ.)
                                    $startDate = new DateTime($leave['leavestart']);
                                    $startDateFormatted = $startDate->format('d/m/Y'); // ใช้ ค.ศ. (ลบการบวก 543)

                                    // แปลงวันที่สิ้นสุดเป็นรูปแบบ d/m/Y (ค.ศ.)
                                    $endDate = new DateTime($leave['leaveend']);
                                    $endDateFormatted = $endDate->format('d/m/Y'); // ใช้ ค.ศ. (ลบการบวก 543)
                                        ...
                                        <td>
                                            <?php if (($leave['leavestatus'] == 'อนุมัติ' && $leave['note'] == 'ยกเลิกลาและคืนวันลา') || ($leave['leavestatus'] == 'อนุมัติ' && $leave['note'] == 'ยกเลิกลา')): ?>
                                                <button type="button" class="btn btn-info btn-sm print-cancel-btn"
                                                    onclick="window.open('print_cancel.php?leavesid=<?= $leave['leavesid'] ?>', '_blank')"
            ...
            $(document).on('click', '.cancel-leave-btn', function() {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: '../user/process/process_cancel_leave.php',
                            type: 'POST',
                            data: {
                                leavesid: leavesid
                            },
